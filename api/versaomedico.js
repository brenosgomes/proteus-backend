module.exports = app => {
    const get = app.get('/vmedico', (req,res)=>{
        mysqlConnection.query('select * from vmedico', (err, rows, fields)=>{
            if(!err)
                res.send(rows)
            else
                console.log(err)
    ...
    const getu = app.get('/vmedico/:id', (req,res)=>{
        mysqlConnection.query('select * from vmedico where pac_id = ?', [req.params.id],(err, rows, fields)=>{
            if(!err)
                res.send(rows);
            else
                console.log(err)
    ...
    const del = app.delete('/vmedico/:id', (req,res)=>{
        mysqlConnection.query('delete from vmedico where pac_id = ?', [req.params.id],(err, rows, fields)=>{
            if(!err)
    ...
    const add = app.post('/vmedico', (req,res)=>{
        ...
        var sql = "SET @vmed_id = ?; SET @vmed_fk_ava = ?; SET @vmed_ava_glo = ?;SET @vmed_pt_tot = ?; SET @vmed_q1_per_pes = ?; SET @vmed_q2_diag_rel = ?;\
                   ...
                   CALL VMedicoAddOrEdit(@vmed_id, @vmed_fk_ava, @vmed_ava_glo, @vmed_pt_tot, @vmed_q1_per_pes, @vmed_q2_diag_rel,\
